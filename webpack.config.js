      ...
      patterns: [
        ...
        },
        {
          ...
          to({ absoluteFilename }) {
            ...
            if(diff.startsWith('templates\\customers\\')) {
              var targetFolder = diff.startsWith('templates\\customers\\')
              ? 'templates/customers/'
              : diff.split(path.sep)[0];
            } else if (diff.startsWith('templates\\metaobject\\')) {
              var targetFolder = diff.startsWith('templates\\metaobject\\')
              ? 'templates/metaobject/'
              : diff.split(path.sep)[0];
            } else {
              var targetFolder = diff.split(path.sep)[0];
            }
